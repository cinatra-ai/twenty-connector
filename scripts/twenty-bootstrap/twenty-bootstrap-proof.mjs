...
// NO bulk cleanup — Twenty MCP (pinned TWENTY_TAG, 2.7.3) has no filter-delete tool; fixture rows
...
// The proof runs AGAINST a cinatra core checkout: the docker compose stack,
// the shared dev-content fixtures, and the Twenty keygen helpers all live in
// cinatra-ai/cinatra. Resolve that checkout from CINATRA_CORE_ROOT (falling
// back to the current working directory) and import the core-owned helpers
// from it. Run it from the core checkout root, e.g.:
//   node <path-to-this-repo>/scripts/twenty-bootstrap/twenty-bootstrap-proof.mjs
const CORE_ROOT = resolve(process.env.CINATRA_CORE_ROOT || process.cwd());
const CORE_HELPERS = {
...
};
{
  const missing = ["docker-compose.yml", ...Object.values(CORE_HELPERS)].filter(
    (rel) => !existsSync(join(CORE_ROOT, rel)),
  );
  if (missing.length > 0) {
    console.error(
      `[proof] FAIL: ${CORE_ROOT} does not look like a cinatra core checkout ` +
        `(missing: ${missing.join(", ")}).\n` +
        `  Run this script from the root of a cinatra-ai/cinatra checkout, or point ` +
        `CINATRA_CORE_ROOT at one.`,
    );
    process.exit(2);
  }
}
const coreImport = (rel) => import(pathToFileURL(join(CORE_ROOT, rel)).href);
const { seedTwentyContent } = await coreImport(CORE_HELPERS.seedContent);
...
// Gitignored (/data/ in the core checkout). Lets re-runs skip/replace without
// duplicating, and honor user deletes/renames. CI runs in-memory only.
const TWENTY_FIXTURE_PROV_PATH = join(CORE_ROOT, "data", "twenty", "dev-content-provenance.local.json");
...
// between minor versions; the pinned 2.7.3 uses `ObjectFilter`, prior versions used
...
async function step10_views(client, nativeTools, catalogToolNames) {
  ...
  try {
    ...
    // semantics differ in the pinned 2.7.3, this step degrades to a WARN with the row count
...
async function cleanup(_client, _nativeTools) {
  ...
  // single-record IDs (no bulk filter delete in MCP catalog as of the pinned 2.7.3).
