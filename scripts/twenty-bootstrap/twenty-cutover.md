...
This tooling lives in the `twenty-connector` repo but operates on a
**cinatra core dev environment** (the `SUPABASE_DB_URL` / `SUPABASE_SCHEMA`
env of a `cinatra-ai/cinatra` checkout). Run the commands below from the core
checkout root — there the connector is cloned back at
`extensions/cinatra-ai/twenty-connector/`, so the script path is
`extensions/cinatra-ai/twenty-connector/scripts/twenty-bootstrap/cutover-wipe-and-reseed.mjs`.

## What it does

`scripts/twenty-bootstrap/cutover-wipe-and-reseed.mjs` (in this repo):
...
```bash
node extensions/cinatra-ai/twenty-connector/scripts/twenty-bootstrap/cutover-wipe-and-reseed.mjs --dry-run
```
...
```bash
node extensions/cinatra-ai/twenty-connector/scripts/twenty-bootstrap/cutover-wipe-and-reseed.mjs --yes
```
